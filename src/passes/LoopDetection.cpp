...
void LoopDetection::discover_loop_and_sub_loops(BasicBlock *bb, BBset &latches,
    ...
    while (!work_list.empty()) { // 当工作表非空时继续处理
        ...
        // TODO-2: 处理已属于其他循环的节点
            ...
            auto top_parent = sub_loop;
            while (top_parent->get_parent() != nullptr) {
                top_parent = top_parent->get_parent();
            }
            if (top_parent.get() == loop.get()) {
                continue;
            }
            sub_loop->set_parent(loop);
            loop->add_sub_loop(sub_loop);
            for (auto blk : sub_loop->get_blocks()) {
                loop->add_block(blk);
                bb_to_loop_[blk] = loop;
            }
