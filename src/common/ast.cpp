  ...
  */
  if (_STR_EQ(n->name, "program")) {
    ...
    */
    ...
    auto param_list_ptr = n->children[3]->children[0]; // param-list
    // 扁平化参数列表
    if (param_list_ptr->children_num != 0) {
      ...
      while (!param_stack.empty()) {
          auto param_node = static_cast<ASTParam *>(transform_node_iter(param_stack.top()));
          auto param_shared = std::shared_ptr<ASTParam>(param_node);
          node->params.push_back(param_shared);
          param_stack.pop();
      }
    }
    
    // 1.2 compound_stmt 字段填充
    // compound_stmt -> { declaration-list statement-list }
    auto compound_stmt_node = static_cast<ASTCompoundStmt *>(transform_node_iter(n->children[5]));
    ...
    */
    ...
    std::stack<syntax_tree_node *> local_decl_stack;
    while (local_decl_ptr->children_num == 2) {
        local_decl_stack.push(local_decl_ptr->children[1]);
        local_decl_ptr = local_decl_ptr->children[0];
    }

    while (!local_decl_stack.empty()) {
        auto local_decl_node = static_cast<ASTVarDeclaration *>(transform_node_iter(local_decl_stack.top()));
        auto local_decl_shared = std::shared_ptr<ASTVarDeclaration>(local_decl_node);
        node->local_declarations.push_back(local_decl_shared);
        local_decl_stack.pop();
    }
    // statement list
    // 2.2 flatten statement-list
    auto stmt_list_ptr = n->children[2];
    std::stack<syntax_tree_node *> stmt_list_stack;
    while (stmt_list_ptr->children_num == 2) {
        stmt_list_stack.push(stmt_list_ptr->children[1]);
        stmt_list_ptr = stmt_list_ptr->children[0];
    }

    while (!stmt_list_stack.empty()) {
        auto stmt_list_node = static_cast<ASTStatement *>(transform_node_iter(stmt_list_stack.top()));
        auto stmt_list_shared = std::shared_ptr<ASTStatement>(stmt_list_node);
        node->statement_list.push_back(stmt_list_shared);
        stmt_list_stack.pop();
    }
    ...
    */
    ...
    if (n->children_num == 7) {
        ...
        node->else_statement = nullptr; // 如果没有 else 语句，设置为 nullptr
    }
    ...
    if (n->children_num == 3) {
      ...
      */
      ...
      // 1. 递归处理左侧的加法表达式
        auto left_expr_node = static_cast<ASTAdditiveExpression *>(transform_node_iter(n->children[0]));
        node->additive_expression = std::shared_ptr<ASTAdditiveExpression>(left_expr_node);

        // 2. 处理操作符
        auto op_node = n->children[1]->children[0];
        if (_STR_EQ(op_node->name, "+")) {
            node->op = OP_PLUS;
        } else if (_STR_EQ(op_node->name, "-")) {
            node->op = OP_MINUS;
        } else {
            _AST_NODE_ERROR_; // 处理错误情况
        }
